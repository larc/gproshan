...
optix::~optix()
{
	...
	for(auto & textures: textures_mesh)
	for(auto & tex: textures)
		cudaFree(tex.data);

	for(scene_data & sd: scene_mesh)
	{
		cudaFree(sd.materials);
		cudaFree(sd.textures);
		cudaFree(sd.trig_mat);
		cudaFree(sd.texcoords);
	}
...
OptixTraversableHandle optix::build_as(const std::vector<const che *> & meshes, const std::vector<mat4> & model_mats)
{
	...
	cudaMalloc(&d_scene_mesh, size(scene_mesh) * sizeof(scene_data));
	cudaMemcpy(d_scene_mesh, scene_mesh.data(), size(scene_mesh) * sizeof(scene_data), cudaMemcpyHostToDevice);
...
{
	che_cuda * d_m = new che_cuda(mesh);
