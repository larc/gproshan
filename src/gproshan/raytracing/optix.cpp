...
optix::optix(const std::string & ptx)
{
	...
	//_module_compile_opt.maxRegisterCount	= 50;
	_module_compile_opt.optLevel			= OPTIX_COMPILE_OPTIMIZATION_DEFAULT;
...
void optix::render(vec4 * img, const render_params & p, const bool flat)
{
	params.depth = p.depth;
	params.n_frames = p.n_frames;
	params.n_samples = p.n_samples;
	params.color_buffer = img;

	params.window_size = p.window_size;
	if(p.viewport_is_window)
		params.window_size = p.viewport_size;

	params.viewport_pos = p.viewport_pos;

	params.flat = flat;
	params.cam_pos = p.cam_pos;
	params.inv_proj_view = p.inv_proj_view;
	params.ambient = p.ambient;
	params.n_lights = p.n_lights;
	memcpy(params.lights, p.lights, sizeof(params.lights));
...
void optix::create_raygen_programs()
{
	...
	pg_desc.raygen.module				= _module_raygen;
...
void optix::create_miss_programs()
{
	...
	pg_desc.miss.module					= _module_radiance;
...
void optix::create_hitgroup_programs()
{
	...
	pg_desc.hitgroup.moduleCH			= _module_radiance;
	pg_desc.hitgroup.moduleAH			= _module_radiance;
