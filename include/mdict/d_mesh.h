...
distance_t mesh_reconstruction(che * mesh, size_t M, const distance_t & radio, std::vector<patch> & patches, std::vector<vpatches_t> & patches_map, a_mat & A, a_mat & alpha, distance_t * dist,  const fmask_t & mask = nullptr, const index_t & v_i = 0);
