...
class geodesics
{
	...
	public:
		geodesics(che * mesh,							///< input mesh must be a triangular mesh.
				...
				real_t *const & e_dist = nullptr,	///< external dist allocation
				const bool & cluster = false,			///< to cluster vertices to closest source.
				const size_t & n_iter = 0, 				///< maximum number of iterations.
				const real_t & radio = INFINITY		///< execute until the specific radio.
				);
	...
	private:
		void execute(che * mesh, const std::vector<index_t> & sources, const size_t & n_iter, const real_t & radio, const option_t & opt);
